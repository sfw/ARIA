# ARIA v3: The Definitive AI-First Systems Language
...
Eliminates 94.8% of Rust's lifetime errors. This is ARIA's killer feature.
...
aria generate --grammar "fn add(a: Int, b: Int) -> Int = ???"

# Generate code constrained to valid types
aria generate --typed "fn ???(items: [Int]) -> Int"

# Generate code that satisfies contracts
aria generate --verified "fn sort<T: Ord>(input: [T]) -> [T] ensures is_sorted(result)"
```
...
aria verify             # Run formal verification
aria fmt                # Format code
aria grammar            # Export grammar (EBNF/JSON)
aria lsp                # Language server
aria new project        # Create new project
aria add dep            # Add dependency
```
...
ARIA v3 is a refinement, not a rewrite. The core innovations (second-class references, AI-first tooling) remain. We're fixing the rough edges (keyword conflicts, missing features) and adding the differentiators (verification, constrained generation) that make ARIA the obvious choice for AI-generated systems code.

The goal: **Make it impossible for AI to generate incorrect ARIA code.**
